...
public class CheckGrid : MonoBehaviour {
	...
	void Update () {
		Collider[] hitColliders = Physics.OverlapSphere (this.transform.position, 0.2f);//get gameobjects right beside this object
		int i = 0;
		while (i < hitColliders.Length) {
			...
			if (hitColliders [i].transform.tag == "Player" && this.tag == "EnemyMonster" && hitColliders [i].GetComponent<TranslateMonster> ().TMonster) {//if this is enemy and collided to player
					hitColliders [i].GetComponent<Stats> ().Attack (this.gameObject);//attack player
					//hitColliders [i].GetComponent<TranslateMonster> ().nextTurn();
				}
